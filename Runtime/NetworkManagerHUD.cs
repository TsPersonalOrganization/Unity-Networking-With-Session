...
namespace UnityEngine.Networking
{
    ...
    public class NetworkManagerHUD : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            else
            {
                if (NetworkServer.active)
                {
                    ...
                    for (int i = 0; i < NetworkServer.connections.Count; i++)
                    {
                        ...
                        if(conn == null)
                        {
                            serverMsg = "conn is null at index ="+i;
                        }else
                        {
                            serverMsg = "hostId="+conn.hostId+" connectionId="+conn.connectionId+" groupId="+conn.groupId+" isConnected="+conn.isConnected;
                        }

                        GUI.Label(new Rect(xpos, ypos, 800,20), serverMsg);
            ...
            if(!NetworkServer.active && manager.IsClientConnected() && ClientScene.localPlayers.Count == 0)
            {
                if(GUI.Button(new Rect(xpos, ypos ,200,20), "Create Player"))
                {
                    ClientScene.AddPlayer(0);
                }
                ypos += spacing;
